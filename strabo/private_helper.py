...
def make_interest_point(ip_id,images,form_title,form_body,form_geo_obj,form_layer,form_icon):
    if ip_id:
...
def make_image(ip_idx,form_image_id,form_file_obj,form_descrip,year,month):
    '''
    if a flask.files object is passed in, then
    '''
    if form_image_id != '':
        image = db.session.query(schema.Images).get(form_image_id)
    else:
        image = schema.Images()

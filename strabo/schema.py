...
class InterestPoints(Base,DataType):
    ...
    '''Distinct values of this field correspond to a distict leaflet layers, which allow the user to
    add or remove all the points corresponding to that layer  (see `Control.Layers <http://leafletjs.com/reference.html#control-layers>`_
    ...
    '''List of images that correspond to the interest point.'''
...
class Images(Base,DataType):
    ...
    '''Contains month and year when the image was taken.'''
